...
import com.edt.ut3.refactored.models.domain.notifications.NotificationChannelInformation
...
class NotificationManagerService(private val context: Context) {
     ...
     * has been created.
     * It's called when the [NotificationManagerService]
     * is created.
     *
     ...
     * channel is created.
     */
    private fun createUpdateChannel() = createUpdateNotificationChannel(
        NotificationChannelInformation.UpdateChannel
    )
     ...
     * channel is created.
     */
    private fun createReminderChannel() = createUpdateNotificationChannel(
        NotificationChannelInformation.ReminderChannel
    )
     ...
     * channel is created.
     */
    private fun createFirebaseChannel() = createUpdateNotificationChannel(
        NotificationChannelInformation.FirebaseChannel
    )
    ...
    private fun scheduleNotification(note: Note) {
        ...
        if (reminder.isActive()) {
            ...
            val time = reminder.getReminderDate()!!.time
            val secondsBeforeFiring = (time - System.currentTimeMillis()) / 1000

            Log.d(this::class.simpleName, "schedule: ${reminder.getReminderDate()}")
